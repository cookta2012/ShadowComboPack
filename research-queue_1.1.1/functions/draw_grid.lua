...
function options(player)
	...
	caption.style.minimal_width = set_colspan * 36
	if not global.showIcon[player.index] then caption.style.minimal_width = caption.style.minimal_width * 2 end
...
function technologies(player)
	...
	for name, tech in pairs(player.force.technologies) do
		if tech.enabled and (not tech.researched or global.showResearched[player.index]) then --checks if the research is enabled and either not completed or if it should show completed.
			if (global.showExtended[player.index] or not tech.upgrade or not all(tech.prerequisites, "upgrade") or any(tech.prerequisites, "researched") or matches(tech.prerequisites, "name", global.researchQ[player.force.name])) then
				...
				if not matches(tech.research_unit_ingredients, "name", t) then --checks if any on the research ingredients match the banned list
					...
					if global.showIcon[player.index] then
						...
						if global.offset_tech[player.index] < row and row <= (rq.research_table_height + global.offset_tech[player.index]) then
							...
							frame3.add{type = "button", name = "rq-add"..name, style = "rq-button"}	--adds the final button that can be clicked on top.
						...
						if global.offset_tech[player.index] < row and row <= (rq.research_table_height + global.offset_tech[player.index]) then
							...
							frame.add{type = "frame", name = name.."frame", style = "rq-tech"..name}
